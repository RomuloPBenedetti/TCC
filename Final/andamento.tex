	...
	\chapter{Desenvolvimento}
        ...
        \section{Modelagem}
            ...
            \subsection{Modelagem do software Automatizador}
                ...
                Feitas as observações de quais são as entradas e saídas do software que será automatizado e considerando que a automatização depende da interação entre ambos, é possível deduzir que a saída do automatizador\footnote{ Nesta seção nos referiremos ao software automatizador como “automatizador” ou apenas software.} precisa conter elementos do mesmo tipo que a entrada do software a ser automatizado e precisa ser direcionada ao software a ser automatizado.

                Analogamente, podemos deduzir que a entrada do automatizador precisa conter elementos do mesmo tipo que a saída do software a ser automatizado. Como o software automatizado desconhece a existência do automatizador, o software automatizador precisa procurar e filtrar, quando necessário, saídas do tipo que o software a ser automatizado gera.

                Além das entradas e saídas necessárias para estabelecer uma interface com o software automatizado, o automatizador também necessita receber informações do usuário e responder ao mesmo.

                Considerando o usuário e o software a ser automatizado, as entradas e saídas do software automatizador podem ser observadas na figura \ref{fig:automatIO}, foram discretizadas em três tipos de entrada e três tipos de saída:
                ...
                \begin{itemize}
                    \item entradas:
                        ...
                        \subitem {\bf eventos}: Eventos do software a ser automatizado, procurados quando necessário durante a automatização.
                        \subitem {\bf dados}: Interações do usuário que fornecem dados ou valores, para a realização da automação, ou dados provenientes da saída do software a ser automatizado, procurados quando necessário durante a automatização. São precedidos da entrada de uma ação quando inseridos pelo usuário e podem resultar em algum evento no automatizador.
                    \item saídas:
                        ...
                        \subitem {\bf eventos}: Eventos ou respostas que buscam informar o usuário de alguma condição atingida no automatizador, decorrente de alguma entrada do usuário, seja ação ou dado, podem necessitar a interação do usuário.
                        \subitem {\bf dados}: Dados produzidos pelo automatizador durante a automação, destinados a entrada de dados do software a ser automatizado, ou dados produzidos por alguma ação do usuário sobre o software de automação, que não são destinados ao software a ser automatizado.
                \end{itemize}

                \begin{figure}[!htb]
                    {\centering
                    \includegraphics[width=1.0\textwidth]{imagens/automatIO}
                    \caption{Descrição das entradas e saídas do automatizador, assim como da definição genéria dos seus componentes internos. As engrenagens simbolizam os componentes de controle do automatizador}
                    \label{fig:automatIO}}
                \end{figure}

                Podemos observar que a entrada de ações por ser realizada pelo usuário, é destinada a componentes do automatizador acessíveis ao usuário, ocorrem nos eventos gerados pelo automatizador para tomar decisões, nos elementos de automação onde são selecionados para montar a descrição da automação, assim como na descrição da automação para alterá-la, ocorrem também nas ações do software automatizador, quando o usuário, por exemplo, decide executar a automação. Irão gerar alguma das saídas previstas no automatizador

                A entrada de eventos ocorre seletivamente durante a automação, onde o software a ser automatizado é observado seletivamente de acordo com regras definidas na descrição da automação em buscas de condições descritas pelo usuário na automação. Quando relevantes para a automação, irão interferir na saída de ações ou dados destinados ao software a ser automatizado.

                Já a entrada de dados pode vir tanto do usuário como do software a ser automatizado, durante a automação. Quando vindos do usuário, são destinados a descrição da automação a ser executada, caso contrário, ocorre seletivamente durante a automação, dependendo de como a automação foi descrita pelo usuário. Podem produzir eventos que necessitam da atenção do usuário, devido a dados incorretos por exemplo.

                Na saída do automatizador as ações são produtos da execução da automação descrita com os elementos de automação, destinadas ao software a ser automatizado. Podem produzir eventos ou dados destinados a entrada do automatizador.

                Os eventos, quando considerados como saída, são mensagens do automatizador destinados ao usuário. São produtos de alguma entrada e indicam a satisfação de alguma condição ou algum problema que necessita atenção, portanto, podendo necessitar a interação do usuário, inserção de dados ou realização de alguma ação.

                Já a saída de dados pode ser tanto oriunda da execução da descrição da automação, destinadas ao software automatizador, como de ações realizadas pelo usuário, que não visam interagir com o software a ser automatizado, por exemplo, salvar a automação em um arquivo neste caso são produtos de alguma ação do usuário.
                ...
                \subsection {Componentes do  software automatizador}
                    ...
                    Por ultimo, temos os elementos de controle do automatizador, descritos na figura \ref{fig:automatIO} como engrenagens, são responsáveis por diversas tarefas dentro do software automatizador:

                    \begin{itemize}
                        \item realizam as ações do software automatizador como executar, salvar, excluir ou carregar uma automatização salva, podendo gerar alguma saída dependendo da ação.
                        \item saídas:
                    \end{itemize}
